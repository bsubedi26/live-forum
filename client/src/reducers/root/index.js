        ...
        threads: updateIn(threads, ['queryResult', 'data'], (data) => {
          return data.map(item => {
            if (item.id === payload.thread_id) {
              const updatedObj = update(item, '_comments', (comments) => [payload].concat(comments))
              return updatedObj
            }
    ...
    case 'SOCKET_COMMENTS_ON_REMOVED': {
      return {
        ...
        threads: updateIn(threads, ['queryResult', 'data'], (data) => {
          return data.map(item => {
            if (item.id === payload.thread_id) {
              const updatedObj = update(item, '_comments', (comments) => comments.filter(comment => comment.id !== payload.id))
              return updatedObj
            }
    ...
    case 'SOCKET_COMMENTS_ON_PATCHED': {
      return {
        ...
        threads: updateIn(threads, ['queryResult', 'data'], (data) => {
          return data.map(item => {
            if (item.id === payload.thread_id) {
              const updatedObj = update(item, '_comments', (comments) => {
